...
def parse_model_settings(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    # general
    parser.add_argument(
        "--img-height", "-H", type=int, metavar="", help="Height of the jet images."
    )
    parser.add_argument(
        "--img-width", "-W", type=int, metavar="", help="Width of the jet images."
    )
    parser.add_argument(
        "--latent-vector-size",
        "-L",
    ...
    parser.add_argument(
        ...
        type=str,
        default="arxiv",
        help="Which model to use. Supported choices: (arxiv, ucsd, cnnjetimg).",
    )
    parser.add_argument(
        "--arxiv-model-batch-norm",
        action="store_true",
        default=False,
    ...
    parser.add_argument(
        ...
        metavar="",
        default=[],
        help="Width of hidden linear layers " "in the unflatten layer of the decoder.",
    )
    parser.add_argument(
        "--decoder-cnn-strides",
        "-dcs",
...
def get_device(arg: argparse.Namespace) -> torch.dtype:
    ...
    if arg.lower() == "cpu":
        ...
        logging.warning(
            f"{arg} is not a recognizable device. " f"Defaulting to {DEFAULT_DEVICE}."
        )
...
def get_dtype(arg: argparse.Namespace) -> torch.dtype:
    ...
    if arg.lower() in ("float32", "float64"):
        ...
        logging.warning(
            f"{arg} is not a recognizable device. " f"Defaulting to {DEFAULT_DTYPE}."
        )
