...
def prepare(
    ...
    maxR: float = 0.5,
    npix: int = 40,
    dtype: torch.device = DEFAULT_DTYPE
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Prepare dataset for CNN Autoencoder'
    )
    parser.add_argument(
        '-j', '--jet_types',
    ...
    parser.add_argument(
        '-p', '--npix', type=int, default=40,
        help="Number of pixels in the images."
    )
    parser.add_argument(
        '-s', '--save-dir',
    ...
    parser.add_argument(
        '--dtype',
        type=get_dtype, metavar='', default=DEFAULT_DTYPE,
        help="Test portion of the data."
    )
    ...
    if len(args.maxRs) != len(args.jet_types):
        raise ValueError(
            "Number of maxRs must be equal to the number of jet types."
        )
    logging.info(f"{args=}")
