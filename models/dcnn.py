...
class DCNN(nn.Module):
    ...
    def __init__(
        ...
        if not (isinstance(hidden_channels, list) or (hidden_channels is None)):
            raise ValueError(
                f"hidden_channels must be a list of int or. Found: {type(hidden_channels)}."
            )
        ...
        if isinstance(strides, list) and (len(strides) == 1):
            strides = strides[0]
        self.strides = (
            strides if isinstance(strides, list) else [strides] * self.num_cnns
        )
        if isinstance(paddings, list) and (len(paddings) == 1):
            paddings = paddings[0]
        self.paddings = (
            paddings if isinstance(paddings, list) else [paddings] * self.num_cnns
        )
        if isinstance(dilations, list) and (len(dilations) == 1):
            dilations = dilations[0]
        self.dilations = (
            dilations if isinstance(dilations, list) else [dilations] * self.num_cnns
        )
        ...
        if isinstance(leaky_relu_negative_slopes, list) and (
            len(leaky_relu_negative_slopes) == 1
        ):
            leaky_relu_negative_slopes = leaky_relu_negative_slopes[0]
        self.leaky_relu_negative_slopes = (
            leaky_relu_negative_slopes
            if isinstance(leaky_relu_negative_slopes, list)
        ...
        if len(self.hidden_channels) == 0:
            self.cnn = nn.Sequential(
                ...
                nn.LeakyReLU(
                    negative_slope=self.leaky_relu_negative_slopes[0], inplace=False
        ...
        # input -> hidden layers -> output
            ...
            output_layer = nn.Sequential(
                ...
                nn.LeakyReLU(
                    negative_slope=self.leaky_relu_negative_slopes[-1], inplace=False
                ).to(self.device, self.dtype),
            )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not isinstance(x, torch.Tensor):
            raise TypeError(
                "Input x is expected to be a torch.Tensor. Found: {type(x)}"
            )
        ...
        if x.dim() == 3:  # 1 channel by assumption
            ...
            raise ValueError(
                "Input tensor x must be 3 or 4 dimensional. " f"Found: {x.dim()=}"
            )
        if not self.keep_intermediates:
            ...
            for layer in self.cnn:
                x = layer(x)
                intermediate_feature_maps.append(x.flatten(start_dim=1))
