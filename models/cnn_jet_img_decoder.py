...
class CNNJetImgDecoder(nn.Module):
    ...
    def __init__(
        ...
        linear_layers_out_dim = (
            cnn_channels[0] * unflatten_img_size[0] * unflatten_img_size[1]
        )
        if (unflatten_hidden_widths is None) or (len(unflatten_hidden_widths) == 0):
            # no hidden layers: latent vector -> image
            linear_layers.append(
                nn.Linear(self.latent_vector_size, linear_layers_out_dim)
            )
        else:
            # input -> hidden layers
            linear_layers.append(
                nn.Linear(self.latent_vector_size, unflatten_hidden_widths[0])
            )
            # hidden layers
            for i in range(len(unflatten_hidden_widths) - 1):
                linear_layers.append(
                    nn.Linear(
                        unflatten_hidden_widths[i], unflatten_hidden_widths[i + 1]
                    )
                )
            # hidden layers -> output
            linear_layers.append(
                nn.Linear(unflatten_hidden_widths[-1], linear_layers_out_dim)
            )
        ...
        self.unflatten = nn.Sequential(
            *linear_layers,
            nn.LeakyReLU(
                negative_slope=unflatten_leaky_relu_negative_slope, inplace=False
            ),
        ...
        self.output_layer = nn.Sequential(
            ...
            nn.LeakyReLU(
                negative_slope=output_leaky_relu_negative_slope, inplace=False
            ),
        ).to(self.device, self.dtype)
